...
/// ```
///
/// [onUploaded] is called with the public URL after a successful upload.
/// Store this URL in your DTO/model and send it to the backend.
class ImagePickerField extends ConsumerStatefulWidget {
  ...
  /// Supabase bucket name — use StorageBucket constants.
  final String bucket;

  /// Storage path for this file — use StoragePath helpers.
  ...
  /// Existing image URL to display (from DB / current model).
  final String? currentUrl;

  /// Size of the avatar/preview box.
  final double size;

  /// Circle for profile photos, rectangle for cover images.
...
class _ImagePickerFieldState extends ConsumerState<ImagePickerField> {
  ...
  String? _localPreviewPath; // show local file immediately before upload
  ...
  Future<void> _pick(ImageSource source) async {
    ...
    final picked = await _picker.pickImage(
      source: source,
      imageQuality: 80, // compress to ~80% — reduces upload size
      maxWidth: 1024,
      maxHeight: 1024,
    );
            ...
            if (_localPreviewPath != null || widget.currentUrl != null) ...[
              ...
              _SheetOption(
                ...
                onTap: () {
                  ...
                  // ref.read(supabaseStorageServiceProvider).delete(
                  //   bucket: widget.bucket, path: widget.storagePath);
                },
              ),
            ],
          ],
  ...
  Widget build(BuildContext context) {
    // Priority: local preview (just picked) > existing URL > placeholder
    final showUrl = _localPreviewPath != null
        ? null // use FileImage below
        : widget.currentUrl;

    final imageProvider = _localPreviewPath != null
        ? FileImage(File(_localPreviewPath!)) as ImageProvider
        : (showUrl != null ? NetworkImage(showUrl) : null);
